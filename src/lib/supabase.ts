...
export async function createUser(username: string): Promise<any> {
  if (!isConfigured) return null;
  const { data } = await supabase.from('users').insert({ username, total_clicks: 0, xp_balance: 0, status: 'active' }).select().single();
  return data;
}
...
export async function getTodayClicks(userId: string): Promise<number> {
  ...
  const { data } = await supabase.from('click_sessions').select('clicks').eq('user_id', userId).gte('timestamp', today.toISOString());
  let total = 0;
  data?.forEach((session) => { total += session.clicks; });
  return total;
}

export async function recordClicks(userId: string, clicks: number): Promise<void> {
  if (!isConfigured) return;
  await supabase.from('click_sessions').insert({ user_id: userId, clicks, timestamp: new Date().toISOString() });
  await supabase.rpc('increment_total_clicks', { user_id: userId, click_count: clicks });
}
...
export async function swapClicksToXP(userId: string, clicks: number, xpRate: number): Promise<number> {
  ...
  await supabase.rpc('decrement_clicks_and_increment_xp', { user_id: userId, clicks_to_remove: clicks, xp_to_add: xpGained });
  await supabase.from('xp_transactions').insert({ user_id: userId, clicks_spent: clicks, xp_received: xpGained });
  return xpGained;
}

export async function getUserTransactions(userId: string, limit = 20): Promise<any[]> {
  if (!isConfigured) return [];
  const { data } = await supabase.from('xp_transactions').select('*').eq('user_id', userId).order('timestamp', { ascending: false }).limit(limit);
  return data || [];
}

export async function getAllUsers(page = 0, perPage = 20): Promise<any[]> {
  if (!isConfigured) return [];
  const { data } = await supabase.from('users').select('*').order('created_at', { ascending: false }).range(page * perPage, (page + 1) * perPage - 1);
  return data || [];
}
